...
def cross_entropy_loss(probs, target_index):
    ...
    '''
    if probs.ndim == 1:
        return -np.log(probs[target_index] + sys.float_info.epsilon)
    else:
        batch_size = probs.shape[0]
        return -np.sum(np.log(probs[np.arange(batch_size), target_index] + sys.float_info.epsilon))
    ...
    def fit(self, X, y, batch_size=100, learning_rate=1e-7, reg=1e-5,
        ...
        for epoch in range(epochs):
            ...
            for batch_indices in batches_indices:
                ...
                loss, grad = linear_softmax(batch, self.W, y[batch_indices])
